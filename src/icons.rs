// Icons from Lucide (https://lucide.dev) — ISC License
// Copyright (c) for portions of Lucide are held by Cole Bemis 2013-2026 as part of Feather (MIT).
// All other copyright (c) for Lucide are held by Lucide Contributors 2026.

pub const VSCODE_ICON: &str =
    r#"<path d="m18 16 4-4-4-4" /><path d="m6 8-4 4 4 4" /><path d="m14.5 4-5 16" />"#;
pub const TERMINAL_ICON: &str = r#"<path d="M12 19h8" /><path d="m4 17 6-6-6-6" />"#;
pub const NOTIFICATION_ICON: &str = r#"<path d="M10.268 21a2 2 0 0 0 3.464 0" /><path d="M3.262 15.326A1 1 0 0 0 4 17h16a1 1 0 0 0 .74-1.673C19.41 13.956 18 12.499 18 8A6 6 0 0 0 6 8c0 4.499-1.411 5.956-2.738 7.326" />"#;
pub const MY_PR_ICON: &str = r#"<circle cx="5" cy="6" r="3" /><path d="M5 9v12" /><circle cx="19" cy="18" r="3" /><path d="m15 9-3-3 3-3" /><path d="M12 6h5a2 2 0 0 1 2 2v7" />"#;
pub const PR_OPEN_ICON: &str = r#"<circle cx="18" cy="18" r="3" /><circle cx="6" cy="6" r="3" /><path d="M13 6h3a2 2 0 0 1 2 2v7" /><line x1="6" x2="6" y1="9" y2="21" />"#;
pub const PR_QUEUED_ICON: &str = r#"<path d="M16 6H5" /><path d="M16 12H5" /><path d="M13 18H5" /><path d="m19 16 2 2-2 2" /><path d="M17 18h4" />"#;
pub const PR_DRAFT_ICON: &str = r#"<circle cx="18" cy="18" r="3" fill="none" stroke-dasharray="2.5 2.5" /><circle cx="6" cy="6" r="3" /><path d="M13 6h3a2 2 0 0 1 2 2v7" /><line x1="6" x2="6" y1="9" y2="21" />"#;
pub const PR_MERGED_ICON: &str = r#"<circle cx="18" cy="18" r="3" /><circle cx="6" cy="6" r="3" /><path d="M6 21V9a9 9 0 0 0 9 9" />"#;
pub const PR_CLOSED_ICON: &str = r#"<circle cx="6" cy="6" r="3" /><path d="M6 9v12" /><path d="m21 3-6 6" /><path d="m21 9-6-6" /><path d="M18 11.5V15" /><circle cx="18" cy="18" r="3" />"#;
pub const ISSUE_OPEN_ICON: &str = r#"<circle cx="12" cy="12" r="10" /><line x1="12" x2="12" y1="8" y2="12" /><line x1="12" x2="12.01" y1="16" y2="16" />"#;
pub const ISSUE_CLOSED_ICON: &str =
    r#"<circle cx="12" cy="12" r="10" /><path d="m9 12 2 2 4-4" />"#;
pub const DISCUSSION_OPEN_ICON: &str =
    r#"<path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z" />"#;
pub const DISCUSSION_ANSWERED_ICON: &str = r#"<path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z" /><path d="m9 10 2 2 4-4" />"#;
pub const CHECKMARK_ICON: &str = r#"<path d="M20 6 9 17l-5-5" />"#;
pub const REFRESH_ICON: &str = r#"<path d="M3 12a9 9 0 0 1 9-9 9.75 9.75 0 0 1 6.74 2.74L21 8" /><path d="M21 3v5h-5" /><path d="M21 12a9 9 0 0 1-9 9 9.75 9.75 0 0 1-6.74-2.74L3 16" /><path d="M8 16H3v5" />"#;
pub const MAIL_ICON: &str = r#"<path d="m22 7-8.991 5.727a2 2 0 0 1-2.009 0L2 7" /><rect x="2" y="4" width="20" height="16" rx="2" />"#;
